...
module.exports = new SmartApp()
    ...
    .subscribedEventHandler('alarmHandler', (context, event) => {
        ...
        if (parent.getSchedStatus(app.id) && !alarmMode || alarmMode.contains(location.mode) && this.getDayOk() && this.getTimeOk(timeStart, timeEnd) && this.everyoneIsPresent() && this.switchesOnStatus() && this.switchesOffStatus()) {
        ...
        state.sound = this.textToSpeech(state.fullMsg, true)
        }
        if (alarmType == '1') {
        ...
        if (alarmType == '3') {
        let text = this.textToSpeech(' ', true)
    ...
    .subscribedEventHandler('alarmHandlerTrigger', (context, event) => {
        
        if (!alarmPresenceTriggerTime || alarmPresenceTriggerTime == 0) {
        this.alarmHandler()
        } else {
        this.runIn(alarmPresenceTriggerTime * 60, alarmHandler, ['overwrite': true])
        }
