...
module.exports = new SmartApp()
    ...
    .subscribedEventHandler('offHandler', (context, event) => {
        ...
        console.log("MCO-2-APP-offHandler $evt")
    ...
    .subscribedEventHandler('onHandler', (context, event) => {
        ...
        console.log("MCO-2-APP-onHandler $evt")
        let endpoint = this.getEndpoint(event.deviceId)
    ...
    .subscribedEventHandler('endpointEvent', (context, event) => {
        ...
        console.log("MCO-2-APP-endpointEvent $evt")
        let values = event.value.split(':')
        let endpoint = values[0]
        let payload = values[1]
        let theswitch = this.getSwitch(endpoint)
        if (payload == '200300') {
        console.log('MCO-2-APP-endpointEvent OFF')
        theswitch.off()
        } else {
        if (payload == '2003FF') {
        console.log('MCO-2-APP-endpointEvent ON')
        theswitch.on()
        } else {
        console.log('MCO-2-APP-endpointEvent ERROR')
        }
        }
