...
module.exports = new SmartApp()
    ...
    .subscribedEventHandler('temperatureHandler', (context, event) => {
        ...
        let retryPeriodInMinutes = retryPeriod ? retryPeriod : 30
        let timeAgo = new Date(this.now() - 1000 * 60 * retryPeriodInMinutes .toLong())
        ...
        this.send("Open some windows to cool down the house! Currently $currentInTemp°F inside and $currentOutTemp°F outside.")
        } else {
        if (currentOutTemp > maxTemp && openWindows ) {
        this.send("It's gotten warmer outside! You should close these windows: ${openWindows.join(, )}. Currently $currentInTemp°F inside and $currentOutTemp°F outside.")
        } else {
        ...
        this.send("Open some windows to warm up the house! Currently $currentInTemp°F inside and $currentOutTemp°F outside.")
        } else {
        if (currentOutTemp < minTemp && openWindows ) {
        this.send("It's gotten colder outside! You should close these windows: ${openWindows.join(, )}. Currently $currentInTemp°F inside and $currentOutTemp°F outside.")
        } else {
        console.log('No notifications sent. Everything is in the right place.')
        }
        }
