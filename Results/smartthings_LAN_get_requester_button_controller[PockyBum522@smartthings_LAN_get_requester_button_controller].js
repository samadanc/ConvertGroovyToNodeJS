                    ...
                    if (firstEventId == event.id) {
                        switch ( buttonNumber ) {
                            case ~('.*1.*') :
                                ...
                                this.sendHttpLAN(URLLocation1)
                                break
                            case ~('.*2.*') :
                                this.executeHandlers(2)
                                this.sendHttpLAN(URLLocation2)
                                break
                            case ~('.*3.*') :
                                this.executeHandlers(3)
                                this.sendHttpLAN(URLLocation3)
                                break
                            case ~('.*4.*') :
                                this.executeHandlers(4)
                                this.sendHttpLAN(URLLocation4)
                                break
                            case ~('.*5.*') :
                                this.executeHandlers(5)
                                this.sendHttpLAN(URLLocation5)
                                break
                            case ~('.*6.*') :
                                this.executeHandlers(6)
                                this.sendHttpLAN(URLLocation6)
                                break
                            case ~('.*7.*') :
                                this.executeHandlers(7)
                                this.sendHttpLAN(URLLocation7)
                                break
                            case ~('.*8.*') :
                                this.executeHandlers(8)
                                this.sendHttpLAN(URLLocation8)
                                break
                        }
                    } else {
                        if (firstEventId == 0) {
