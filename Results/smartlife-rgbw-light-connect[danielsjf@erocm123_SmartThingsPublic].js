...
module.exports = new SmartApp()
    ...
    .subscribedEventHandler('physicalHandler', (context, event) => {
        ...
        if (this.getChildDevice("${it.deviceNetworkId}/${app.id}/$i")) {
    ...
    .subscribedEventHandler('virtualHandler', (context, event) => {
        ...
        this.getChildDevices().each({
        if (event.deviceId == it.id) {
        if (event.value == 'off' && settings["${it.deviceNetworkId.split(/)[0]}_programs_${it.deviceNetworkId.split(/)[2]}_off"]?.toBoolean()) {
