...
module.exports = new SmartApp()
    ...
    .updated(async (context, updateData) => {
        ...
        await context.api.subscriptions.subscribeToDevices(context.config.thermostats, 'thermostat', 'thermostatOperatingState', 'handleThermostatEvent')
    ...
    .subscribedEventHandler('handleThermostatEvent', (context, event) => {
        
        console.log(event.value)
        this.sendValue(evt, {
        it.value == 'heating' ? 1 : it.value == 'cooling' ? -1 : 0
        })
    ...
    .scheduledEventHandler('processQueue', (context, event) => {
        ...
        let header = ['X-Forwarded-For': app.id]
        if (state.queue != []) {
        console.log("Events: ${state.queue}")
        try {
        this.httpPutJson(['uri': url , 'header': header , 'body': state.queue], { let response ->
