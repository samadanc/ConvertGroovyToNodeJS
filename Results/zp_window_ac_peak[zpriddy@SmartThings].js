...
module.exports = new SmartApp()
    ...
    .page('mainPage', (context, page, configData) => {
        ...
        page.section('Pre-Cooldown for peak time when this switch is on...', section => {
            section.deviceSetting('prepeak').capability(['switch']).name('Enable Switch');

        });


        page.section('Pre-Peak Cool down temperature...', section => {
            section.numberSetting('temperatureLowPrePeak').name('Temperature?');

        });


        page.section('Peak time when this switch is on...', section => {
            section.deviceSetting('peak').capability(['switch']).name('Enable Switch');

        });


        page.section('Peak time high temperature...', section => {
            section.numberSetting('temperatureHighPeak').name('Temperature?');

        });


        page.section('Peak time low temperature...', section => {
            section.numberSetting('temperatureLowPeak').name('Temperature?');
    ...
    .updated(async (context, updateData) => {
        ...
        await context.api.subscriptions.subscribeToDevices(context.config.peak, 'switch', 'switch', 'peakHandler')
        ...
        await context.api.subscriptions.subscribeToDevices(context.config.prepeak, 'switch', 'switch', 'peakHandler')

    })

    .subscribedEventHandler('peakHandler', (context, event) => {
        
        if
        switch1?.on()
        console.log('Turnning on for pre-peak time')
        }
        if
        switch1?.off()
        console.log('Turnning off for peak time')
        }
    ...
    .subscribedEventHandler('doorHandler', (context, event) => {
        ...
        if
        state.forceOff = True
        console.log('Fored Off = True')
        } else {
        this.runIn(delayMinutes * 60, turnOffDoor)
        console.log('Runnning door check in 60 seconds')
        }
        } else {
    ...
    .subscribedEventHandler('temperatureHandler', (context, event) => {
        ...
        if
        console.log('Using Pre-Peak Temp Setting for Low Temp')
        tooCold = temperatureLowPrePeak
        }
        if
        console.log('Using Peak Temp Setting for High and Low Temp')
