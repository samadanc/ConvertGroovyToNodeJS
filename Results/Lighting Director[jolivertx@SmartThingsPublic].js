...
module.exports = new SmartApp()
    ...
    .subscribedEventHandler('onEventD', (context, event) => {
        ...
        if (this.getInputOk(D_motion, D_contact, D_lock)) {
        if (!D_triggerOnce || D_triggerOnce && !state.D_triggered && !D_switchDisable || D_switchDisable && !state.D_triggered) {
        console.log("Motion, Door Open or Unlock Detected Running '$ScenarioNameD'")
        ...
        console.log('Motion, Contact or Unlock detected outside of mode or time/date/trigger restriction.  Not running scenario.')
        }
    ...
    .subscribedEventHandler('onEventB', (context, event) => {
        ...
        if (this.getInputOk(B_motion, B_contact, B_lock)) {
        if (!B_triggerOnce || B_triggerOnce && !state.B_triggered && !B_switchDisable || B_switchDisable && !state.B_triggered) {
        console.log("Motion, Door Open or Unlock Detected Running '$ScenarioNameB'")
    ...
    .subscribedEventHandler('onEventA', (context, event) => {
        ...
        if (this.getInputOk(A_motion, A_contact, A_lock)) {
    ...
    .subscribedEventHandler('onEventC', (context, event) => {
        ...
        if (this.getInputOk(C_motion, C_contact, C_lock)) {
        if (!C_triggerOnce || C_triggerOnce && !state.C_triggered && !C_switchDisable || C_switchDisable && !state.C_triggered) {
        console.log("Motion, Door Open or Unlock Detected Running '$ScenarioNameC'")
