...
module.exports = new SmartApp()
    ...
    .scheduledEventHandler('initAutoApp', (context, event) => {
                ...
                let restoreId = settings['restoreId']
                let restoreComplete = settings['restoreCompleted'] == true ? true : false
                if (settings['watchDogFlag']) {
                    ...
                    if (settings['remDiagFlag']) {
                        ...
                        if (restoreId != null && restoreComplete == false) {
                            this.LogAction("Restored AutomationType: (${settings?.automationTypeFlag})", 'info', true)
                            if (parent?.callRestoreState(app, restoreId)) {
                                this.finishFixState(true)
                                parent?.postChildRestore(restoreId)
                                if (parent?.keepBackups() != true) {
                                    parent?.removeAutomationBackupData(restoreId)
                                }
                                this.settingUpdate('restoreCompleted', true, 'bool')
                            }
                        }
                    }
                }
