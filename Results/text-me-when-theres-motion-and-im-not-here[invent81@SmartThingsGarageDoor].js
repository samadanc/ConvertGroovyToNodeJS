...
module.exports = new SmartApp()
    ...
    .subscribedEventHandler('motionActiveHandler', (context, event) => {
        ...
        console.log("$motion1 has moved, sending notifications to: ${recipients?.size()}")
        this.sendNotificationToContacts("${motion1.label} ${motion1.name} moved", recipients)
        } else {
        console.log("$motion1 has moved, sending text")
        this.sendSms(phone1, "${motion1.label} ${motion1.name} moved")
        }
        }
        } else {
