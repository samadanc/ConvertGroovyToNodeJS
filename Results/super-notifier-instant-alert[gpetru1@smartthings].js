    ...
    .subscribedEventHandler('eventHandler', (context, event) => {
                ...
                if (frequency) {
                    ...
                    if (lastTime == null || this.now() - lastTime >= frequency * 60000) {
                        if (parent.loggingOn) {
                            console.log('frequency used and it is time for new message, checking if within time & date period')
                        }
                        if (allOk) {
                    ...
                    if (parent.loggingOn) {
                        console.log('frequency not used, checking if within time & date period')
                    }
                    if (allOk) {
                        this.createInstantMessage(event.name, event.value, event.device)
                    }
