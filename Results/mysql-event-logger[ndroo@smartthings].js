...
module.exports = new SmartApp()
    ...
    .page('mainPage', (context, page, configData) => {
        ...
        page.section('Log these valves:', section => {
            section.deviceSetting('valves').capability(['valve']).name('');
        ...
        page.section('Log these power meters:', section => {
            section.deviceSetting('energymeters').capability(['powerMeter']).name('');

        });


        page.section('Log these door locks:', section => {
            section.deviceSetting('locks').capability(['lock']).name('');

        });


        page.section('REST Server', section => {
            section.textSetting('rest_url').name('API host / IP');
            section.textSetting('rest_key').name('Self generated authentication key');
