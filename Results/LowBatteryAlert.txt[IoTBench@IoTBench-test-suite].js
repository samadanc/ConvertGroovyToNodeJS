...
module.exports = new SmartApp()
    ...
    .page('mainPage', (context, page, configData) => {
        ...
        page.section('Monitoring group $i', section => {
    ...
    .scheduledEventHandler('check_batteries', (context, event) => {
        ...
        let threshold
        let value
        for (java.lang.Integer i = 1; i <= 4; i++) {
        size = settings["group_$i"]?.size() ? settings["group_$i"]?.size() : 0
        if (size > 0) {
        threshold = settings."threshold_$i".toInteger()
        console.log("Checking batteries for group $i (threshold $threshold)")
        ...
        console.log("Group $i has no devices ($size)")
        }
        }
