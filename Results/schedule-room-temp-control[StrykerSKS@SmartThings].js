...
module.exports = new SmartApp()
    ...
    .subscribedEventHandler('ventTemperatureHandler', (context, event) => {
                ...
                if (currentHVACMode == 'heat' || currentHVACMode == 'auto' && ventTemp >= MAX_TEMP_VENT_SWITCH ) {
                    this.open_all_vents()
                    this.send("ScheduleRoomTempControl>current HVAC mode is $currentHVACMode, found one of the vents' value too hot (${event.value}°), opening all vents to avoid any damage")
                }
                if (currentHVACMode == 'cool' || currentHVACMode == 'auto' && ventTemp <= MIN_TEMP_VENT_SWITCH ) {
                    this.open_all_vents()
                    this.send("ScheduleRoomTempControl>current HVAC mode is $currentHVACMode, found one of the vents' value too cold (${event.value}°), opening all vents to avoid any damage")
                }
