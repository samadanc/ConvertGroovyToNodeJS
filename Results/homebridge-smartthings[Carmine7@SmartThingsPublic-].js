...
module.exports = new SmartApp()
    ...
    .updated(async (context, updateData) => {
        ...
        await context.api.subscriptions.subscribeToDevices(context.config.location, 'location', 'hsmAlert', 'changeHandler')
        ...
        await context.api.subscriptions.subscribeToDevices(context.config.location, 'location', 'hsmStatus', 'changeHandler')
        ...
        await context.api.subscriptions.subscribeToDevices(context.config.location, 'location', 'hsmRules', 'changeHandler')

        await context.api.subscriptions.subscribeToDevices(context.config.location, 'location', 'routineExecuted', 'changeHandler')

        await context.api.subscriptions.subscribeToDevices(context.config.location, 'location', 'hsmSetArm', 'changeHandler')
    ...
    .subscribedEventHandler('lanEventHandler', (context, event) => {
                ...
                try {
                    Map msgData = [:]
                    if (headerMap?.size()) {
                        if (headerMap?.evtSource && headerMap?.evtSource == "Homebridge_${this.platform()}") {
                            if (msg?.body != null) {
                                ...
                                if (headerMap?.evtType) {
                                    switch (headerMap?.evtType) {
                                        case 'hkCommand':
                                            let val1 = msgData?.value1 ? msgData?.value1 : null
                                            let val2 = msgData?.value2 ? msgData?.value2 : null
