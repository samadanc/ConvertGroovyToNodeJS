...
module.exports = new SmartApp()
    ...
    .subscribedEventHandler('responseHandler', (context, event) => {
                ...
                let d = this.getChildDevices()?.find({ 
                    it.getDataValue('ip') == parsedEvent?.ip && it.getDataValue('port') == parsedEvent?.port || it.getDataValue('mac') == parsedEvent?.mac && parsedEvent?.headers?.sid == null || it.deviceNetworkId == parsedEvent?.headers?.sid
                })
                if (d) {
