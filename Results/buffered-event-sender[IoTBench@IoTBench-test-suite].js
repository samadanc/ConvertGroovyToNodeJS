...
module.exports = new SmartApp()
    ...
    .updated(async (context, updateData) => {
        ...
        context.api.schedules.runEvery15Minutes('flushBuffer', delay);

    })

    .scheduledEventHandler('flushBuffer', (context, event) => {
        
        let eventBuffer = atomicState.eventBuffer
        log.trace('About to flush the buffer on schedule')
        if (eventBuffer != null && eventBuffer.size() > 0) {
        atomicState.eventBuffer = []
        this.tryShipEvents(eventBuffer)
        }
